...
export async function discoverMcpTools(
  ...
  try {
    ...
    // 🚀 性能优化：顺序启动MCP服务器而不是并行启动
    // 虽然并行启动看起来更快，但会同时阻塞事件循环，导致UI卡顿
    // 顺序启动可以在每个服务器启动后让出事件循环，保持UI响应
 ...
 * @param connectTimeoutMs Optional connection timeout in milliseconds. Defaults to MCP_CONNECT_TIMEOUT_MSEC.
...
export async function connectToMcpServer(
  ...
  mcpServerConfig: MCPServerConfig,
  debugMode: boolean,
  connectTimeoutMs: number = MCP_CONNECT_TIMEOUT_MSEC,
): Promise<Client> {
...
/**
 * Internal implementation of MCP server connection with all error handling logic.
 */
async function _connectToMcpServerInternal(
  mcpServerName: string,
  mcpServerConfig: MCPServerConfig,
  debugMode: boolean,
): Promise<Client> {
