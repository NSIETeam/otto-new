...
export async function runMarketJobs(
  ...
  let cursor = '';
  for (;;) {
    const batch = await deps.repository.transaction(async (tx) => {
      const rows = await tx.all(
        'SELECT * FROM park_market_listings WHERE id>? ORDER BY id LIMIT 250',
        [cursor],
      );
      ...
      return { last: String(rows.at(-1)?.id ?? ''), count: rows.length };
    });
    if (batch.count < 250) break;
    cursor = batch.last;
  }
  await deps.repository.transaction(async (tx) => {
    const oldReports = await tx.all(
      'SELECT * FROM park_market_reports WHERE closed_at IS NOT NULL AND closed_at<=?',
      [now - 180 * MARKET_DAY],
    );
    ...
    for (const appeal of await tx.all(
      'SELECT * FROM park_market_appeals WHERE closed_at IS NOT NULL AND closed_at<=?',
      [now - 180 * MARKET_DAY],
    )) {
    ...
    const events = await tx.all(
      'SELECT * FROM park_market_outbox WHERE delivered_at IS NULL',
    );
