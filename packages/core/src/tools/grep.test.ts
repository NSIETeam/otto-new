...
describe('GrepTool', () => {
  ...
  describe('validateToolParams', () => {
    ...
    it('should accept file paths for single file search', async () => {
  ...
  describe('Custom file type support (tsx, jsx)', () => {
    beforeEach(async () => {
      // Create test files with tsx and jsx extensions
      await fs.writeFile(
        path.join(tempRootDir, 'Component.tsx'),
        'export const MyComponent = () => <div>Hello TSX</div>;',
      );
      await fs.writeFile(
        path.join(tempRootDir, 'OldComponent.jsx'),
        'export const OldComponent = () => <div>Hello JSX</div>;',
      );
    });

    it('should find matches in tsx files using type filter', async () => {
    ...
    it('should find matches in jsx files using type filter', async () => {
    ...
    it('should convert tsx type to glob pattern internally', async () => {
      const params: GrepToolParams = { pattern: 'Component', type: 'tsx' };
      const description = grepTool.getDescription(params);
      expect(description).toContain('Component');
      expect(description).toContain('(tsx files)');
    });
  });
});
