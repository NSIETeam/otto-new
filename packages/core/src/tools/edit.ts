...
import { performAutoLintCheck } from './helpers/autoLintChecker.js';
import {
  BaseTool,
  Icon,
...
**Multiple replacements:** Set \`expected_replacements\` to the number of occurrences you want to replace. The tool will replace ALL occurrences that match \`old_string\` exactly. Ensure the number of replacements matches your expectation.`,
  ...
  async shouldConfirmExecute(
    ...
    // 用于显示的内容需要规范化为LF，避免CLI显示异常
    const currentContentForDisplay = (editData.currentContent ?? '').replace(/\r\n/g, '\n');
    const newContentForDisplay = editData.newContent.replace(/\r\n/g, '\n');
    ...
    const confirmationDetails: ToolEditConfirmationDetails = {
      ...
      fileName,
      fileDiff,
      originalContent: currentContentForDisplay,
      newContent: newContentForDisplay,
  ...
  async execute(
    ...
    try {
      ...
      if (editData.isNewFile) {
        ...
        // 用于显示的内容需要规范化为LF，避免CLI显示异常
        // 即使文件实际写入的是CRLF（如Windows脚本），显示时也使用LF
        const currentContentForDisplay = (editData.currentContent ?? '').replace(/\r\n/g, '\n');
        const newContentForDisplay = editData.newContent.replace(/\r\n/g, '\n');
      ...
      // 🎯 自动执行lint检查
      const lintResult = await performAutoLintCheck(params.file_path, this.config);

      let finalLlmContent = llmSuccessMessageParts.join(' ');
      let finalDisplayResult = displayResult;

      if (lintResult.shouldAppend) {
        finalLlmContent += `\n\n${lintResult.lintMessage}`;

        if (typeof finalDisplayResult === 'object' && finalDisplayResult !== null) {
          finalDisplayResult = {
            ...finalDisplayResult,
            lintStatus: lintResult.lintStatus,
      ...
      return {
        ...
        returnDisplay: finalDisplayResult,
      };
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
      return {
