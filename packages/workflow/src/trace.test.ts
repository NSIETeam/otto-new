...
import { canCreateFileSymlinks } from './test-support/file-symlink-capability.js';
...
function event(
  runId: string,
  summary: string,
): Omit<WorkflowTraceEvent, 'eventId' | 'timestamp'> {
...
describe('FileWorkflowTraceSink', () => {
  it('serializes concurrent appends and rolls a run to a valid bounded JSONL tail', async () => {
    ...
    await Promise.all(
      Array.from({ length: 20 }, (_, index) =>
        sink.append(event(runId, `event-${index}-${'x'.repeat(80)}`)),
      ),
    );
    ...
    expect(new Set(records.map((record) => record.eventId)).size).toBe(
      records.length,
    );
  });
  ...
  it('fails closed instead of deleting a recent trace at the file cap', async () => {
    ...
    await sink.append(
      event('wf-00000000-0000-0000-0000-000000000001', 'recent'),
    );
    await expect(
      sink.append(event('wf-00000000-0000-0000-0000-000000000002', 'new')),
    ).rejects.toThrow('no expired trace');
    expect(await readdir(root)).toHaveLength(1);
  });

  it.runIf(fileSymlinksSupported)(
    'rejects a symlink trace target instead of writing outside the trace root',
    async () => {
      const root = await mkdtemp(
        path.join(os.tmpdir(), 'otto-workflow-trace-'),
      );
      roots.push(root);
      ...
      await expect(sink.append(event(runId, 'unsafe'))).rejects.toThrow(
        'unsafe',
      );
