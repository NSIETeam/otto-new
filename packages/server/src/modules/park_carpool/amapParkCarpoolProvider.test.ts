...
describe('resolveAmapWebServiceKey', () => {
  it('does not use a bundled credential when deployment configuration is absent', async () => {
    expect(resolveAmapWebServiceKey({})).toBe('');
    const fetchImpl = vi.fn();
    const provider = createAmapParkCarpoolProvider({ key: resolveAmapWebServiceKey({}), fetchImpl });
    expect(provider.configured).toBe(false);
    await expect(provider.searchPlaces('北控宏创科技园')).rejects.toThrow('地图服务尚未配置');
