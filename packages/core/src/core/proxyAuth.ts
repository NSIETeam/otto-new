...
export class ProxyAuthManager {
   ...
   * 格式: Otto/<client>/<version> (<platform>; <arch>)
   */
  ...
  private constructor() {
    ...
    if ((process.env.OTTO_SERVER_URL ?? process.env.OTTO_SERVER_URL)?.includes('localhost')) {
      ...
      this.userInfoFilePath = path.join(os.homedir(), '.otto-user', 'user-info.json');
      this.jwtTokenFilePath = path.join(os.homedir(), '.otto-user', 'jwt-token.json');
      this.usageStatsFilePath = path.join(os.homedir(), '.otto-user', 'usage-stats.json');
    }
