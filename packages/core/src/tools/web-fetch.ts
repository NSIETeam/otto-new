...
---`;
      ...
      console.log(`[WebFetchTool] Using temporary chat for fallback processing with full API monitoring`);
      ...
      const fallbackData = await temporaryChat.sendMessage(
        {
        ...
        `webfetch-fallback-${Date.now()}`,
        SceneType.WEB_FETCH
      );

      let resultText = getResponseText(fallbackData) || '';
      ...
      return {
        ...
        returnDisplay: `Content for ${url} processed using fallback fetch.${isTruncated ? ' (Content truncated)' : ''}`,
      };
    } catch (e) {
  ...
  async execute(
    ...
    try {
      ...
      // 设置URL上下文工具
      temporaryChat.setTools([{ urlContext: {} }]);

      const geminiResponse = await temporaryChat.sendMessage(
        {
          message: userPrompt,
          config: {
            abortSignal: signal
...
${sourceListFormatted.join('\n')}`;
      ...
      return {
        llmContent,
        returnDisplay: `Content processed from prompt.${isTruncated ? ' (Content truncated)' : ''}`,
      };
    } catch (error: unknown) {
