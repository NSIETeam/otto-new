...
describe('report/dashboard 路由基本可达', () => {
  ...
  it('平台管理页通过手动令牌创建并列出多个企业，不在 HTML 中泄露平台令牌', async () => {
    ...
    expect(html).toContain('id="platformParkRemove"');
    expect(html).toContain('撤销产业园认证');
    expect(html).toContain('id="deleteOrganization"');
    expect(html).toContain('删除企业');
...
describe('B2B 企业隔离、邀请码与 Token 用量 API', () => {
  ...
  it('platform can revoke park certification and safely delete an enterprise', async () => {
    const { base } = await startIsolated(ADMIN_TOKEN, null);
    const db = await import('./db.js');
    const provision = async (input: {
      ...
      const response = await fetch(`${base}/enterprise/organizations`, {
        method: 'POST',
        headers: {
          'x-otto-admin-token': ADMIN_TOKEN,
          'content-type': 'application/json',
        ...
        body: JSON.stringify({
          ...
          admin: {
            ...
            password: 'platform-delete-test-password',
            name: `${input.name}管理员`,
          },
        }),
      });
    ...
    const owner = await provision({
      name: '可撤销产业园',
      slug: 'revocable-park-owner',
      username: 'revocable.park.owner',
    });
    const tenant = await provision({
      name: '待解绑企业',
      slug: 'detachable-tenant',
      username: 'detachable.tenant.owner',
    });
    const parkResponse = await fetch(
      `${base}/enterprise/platform/organizations/${encodeURIComponent(owner.organization.id)}/park`,
      {
        method: 'POST',
        headers: {
          'x-otto-admin-token': ADMIN_TOKEN,
        ...
        body: JSON.stringify({ name: '可撤销产业园', brandName: '可撤销园区服务' }),
      },
    );
    expect(parkResponse.status).toBe(201);
    const park = (await parkResponse.json()).park;
    const invite = db.issueParkInvite({
      parkId: park.id,
      actorAccountId: owner.admin.id,
    });
    db.joinOrganizationToPark({
      organizationId: tenant.organization.id,
      actorAccountId: tenant.admin.id,
      code: invite.code,
      address: '科技大厦 A 座',
      roomNumber: '1203 室',
    });

    const ownerSession = db.createAuthSession(owner.admin.id).token;
    const accountCannotRevoke = await fetch(
      `${base}/enterprise/platform/organizations/${encodeURIComponent(owner.organization.id)}/park`,
      {
        method: 'DELETE',
    ...
    expect(accountCannotRevoke.status).toBe(403);
    ...
    expect(await revoke.json()).toMatchObject({
      parkId: park.id,
      detachedOrganizationCount: 1,
      removed: true,
    });
    expect(db.getPark(park.id)?.status).toBe('disabled');
    expect(db.getOrganization(owner.organization.id)?.parkId).toBeNull();
    expect(db.getOrganization(tenant.organization.id)?.parkId).toBeNull();
    expect(db.getParkForOrganization(owner.organization.id)).toBeNull();
    expect(db.getParkForOrganization(tenant.organization.id)).toBeNull();
    ...
    const restore = await fetch(
      `${base}/enterprise/platform/organizations/${encodeURIComponent(owner.organization.id)}/park`,
      {
        ...
        body: JSON.stringify({ name: '重新认证产业园', brandName: '重新认证园区服务' }),
      },
    );
    expect(restore.status).toBe(201);
    expect((await restore.json()).park).toMatchObject({ id: park.id, status: 'active' });

    const accountCannotDelete = await fetch(
      `${base}/enterprise/platform/organizations/${encodeURIComponent(owner.organization.id)}`,
      {
        method: 'DELETE',
        headers: { authorization: `Bearer ${ownerSession}` },
      },
    );
    expect(accountCannotDelete.status).toBe(403);

    const removeOrganization = await fetch(
      `${base}/enterprise/platform/organizations/${encodeURIComponent(owner.organization.id)}`,
      {
        method: 'DELETE',
        headers: { 'x-otto-admin-token': ADMIN_TOKEN },
    ...
    expect(removeOrganization.status).toBe(200);
    expect(await removeOrganization.json()).toMatchObject({
      id: owner.organization.id,
      parkCertificationRemoved: true,
      deleted: true,
    });
    expect(db.getOrganization(owner.organization.id)?.status).toBe('disabled');
    expect(db.getAccountBySession(ownerSession)).toBeNull();
    expect(db.listEnterpriseOrganizations().map((organization) => organization.id))
      .not.toContain(owner.organization.id);

    const deletedOverview = await fetch(
      `${base}/enterprise/platform/organizations/${encodeURIComponent(owner.organization.id)}/overview`,
      { headers: { 'x-otto-admin-token': ADMIN_TOKEN } },
    );
    expect(deletedOverview.status).toBe(404);
  });
