...
describe('official QR pairing brokers', () => {
  it.each(['cancel', 'expiry'] as const)('discards WeCom credentials returned after %s', async (end) => {
    const pendingRegistration = registration('wecom');
    let now = Date.now();
    let finishPoll!: (value: Response) => void;
    const fetchImpl = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify({ data: { scode: 'code', auth_url: 'https://work.weixin.qq.com/ai/qc/gen' } })))
      .mockImplementationOnce(() => new Promise((resolve) => { finishPoll = resolve; }));
    const broker = new WeComOfficialQrPairingBrokerV1({ fetchImpl, now: () => now });
    await broker.register(pendingRegistration);
    const polling = broker.poll(pendingRegistration.pairingId);
    if (end === 'cancel') await broker.cancel(pendingRegistration.pairingId);
    else now = pendingRegistration.expiresAtMs;
    finishPoll(new Response(JSON.stringify({ data: { status: 'success', bot_info: { botid: 'bot', secret: 'late-secret' } } })));
