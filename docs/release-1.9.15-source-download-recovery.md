...
## HTTP content-coding follow-up (2026-09-10)

Formal run `34421815423`, source `c5ba104a7ab777181e8c8bc36f30986143b64d37`, passed all 25 fresh preflight jobs but failed before packaging at `source content length mismatch`. The exact historical input is unknown because this error did not include the filename. Original job-log SHA256: `a238250061e4b64873febab4b848f9ff9c43687fafcf91df1b4480bed378d8b3`. All 12 downstream jobs were skipped; no installer publication or production deployment occurred. Both temporary publication OAuth Secret copies were removed after terminal failure.

A real Node 22.23.1 request to the fixed Mozilla MPL text subsequently returned Brotli encoding and a wire `Content-Length` of 5,012, while Fetch delivered 16,726 decoded bytes matching the unchanged manifest and SHA256 `3f3d9e0024b1921b067d6f7f88deb4a60cbe7a78e76c64e3f1d7fc3b779b9d04`. The old header comparison rejects this correct file. This proves the defect class, not the identity of the historical failing request. Separate local certificate-chain failures remain failures; TLS validation was not disabled.

The downloader now requests identity transfer and handles a server's gzip/deflate/Brotli response without equating encoded length to decoded artifact length. Any declared length must be a bounded unsigned integer; identity responses still require the exact declared size. Unknown or stacked encodings fail closed. Streamed decoded bytes still cannot exceed the pinned size or 64 MiB, and exact final size, SHA256 and applicable npm SHA512 remain mandatory. URLs, versions, expected hashes, allowed hosts, four-redirect limit and 60-second deadline are unchanged. Response failures identify only the fixed reason, public filename and hostname, never upstream headers, bodies or signed query strings, and unread bodies are canceled.

The new transport suite first reproduced 14 failures and one pass. After correction, all 31 transport and existing corresponding-source tests passed, including actual loopback Node Fetch gzip/deflate/Brotli decoding, identity/headerless downloads, malformed lengths, unsupported encodings, oversized/truncated/corrupt decoded content, and signed-query redaction. The loopback adapter is test-only; production still requires the reviewed HTTPS source. This is downloader evidence, not installer, deployment or release acceptance.

The full script suite then passed 696 tests with zero failures and four existing platform skips. Independent review requested an explicit body-reader error redaction case; that additional case and the original related tests passed together (32 tests, zero failures). Doctor, focused ESLint, code-map and whitespace checks passed. No coverage baseline, verifier threshold, dependency pin, product module or deployment script changed. A real call through the corrected downloader also fetched the exact pinned Mozilla bytes successfully; the separate all-input network audit retains any local certificate failures rather than bypassing TLS or substituting cached successes.
