...
OTTO_AMAP_WEB_SERVICE_KEY="${OTTO_AMAP_WEB_SERVICE_KEY:-}"
OTTO_PARK_CARPOOL_MINIMUM_OVERLAP="${OTTO_PARK_CARPOOL_MINIMUM_OVERLAP:-0.35}"
OTTO_PARK_CARPOOL_REQUESTS_ENABLED="${OTTO_PARK_CARPOOL_REQUESTS_ENABLED-false}"
OTTO_PARK_CARPOOL_INVITATIONS_ENABLED="${OTTO_PARK_CARPOOL_INVITATIONS_ENABLED-false}"
OTTO_PARK_CARPOOL_GROUPS_ENABLED="${OTTO_PARK_CARPOOL_GROUPS_ENABLED-false}"
...
[[ "$OTTO_PARK_CARPOOL_MINIMUM_OVERLAP" =~ ^(0([.][0-9]+)?|1([.]0+)?)$ ]] \
  || otto_die "OTTO_PARK_CARPOOL_MINIMUM_OVERLAP 必须是 0 到 1 之间的数字"
...
NODE_PATH="$(otto_resolve_node "$PREFERRED_NODE" || true)"
if [ -z "$NODE_PATH" ] \
  || [ "$("$NODE_PATH" --version)" != "v${OTTO_NODE_VERSION}" ]; then
  if ! command -v curl >/dev/null 2>&1 \
    || [ ! -r /etc/ssl/certs/ca-certificates.crt ]; then
    if [ "$DRY_RUN" -eq 1 ]; then
      otto_die "dry-run 深度校验需要 Node v${OTTO_NODE_VERSION}，或预先安装 curl 与 ca-certificates 以下载临时固定 runtime" 3
    fi
...
if [ "$DRY_RUN" -eq 1 ]; then
  otto_log "dry-run 深度校验通过：包清单、release、迁移归档、SQLCipher、schema 与数据对账均通过"
  printf '  迁移后账号数：%s\n' "$ACCOUNT_COUNT"
...
# Preserve the runtime's distinction between absent and explicitly configured.
# In particular, an absent pilot list must not become an invalid empty string.
# Values remain literal; the runtime's existing validation remains authoritative.
CARPOOL_RUNTIME_ENV_ARGS=()
for carpool_key in \
...
write_env "$ENV_TEMP" \
  ...
  OTTO_FEDERATION_SIGNING_KEY_FILE "$OTTO_FEDERATION_SIGNING_KEY_FILE" \
  OTTO_AMAP_WEB_SERVICE_KEY "$OTTO_AMAP_WEB_SERVICE_KEY" \
  OTTO_PARK_CARPOOL_MINIMUM_OVERLAP "$OTTO_PARK_CARPOOL_MINIMUM_OVERLAP" \
  OTTO_DATA_CONTROLLER_NAME "$OTTO_DATA_CONTROLLER_NAME" \
  OTTO_PRIVACY_CONTACT "$OTTO_PRIVACY_CONTACT" \
