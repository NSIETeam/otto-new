...
function parseUnifiedDiff(patchText: string): { hunks: Hunk[] } {
    ...
    while (i < lines.length) {
        ...
        if (line.startsWith("--- ")) {
            ...
            const filePath = isDevNullOld ? newPath : oldPath;
...
function seekSequence(lines: string[], pattern: string[], startIndex: number): number {
    ...
    const normalizeString = (s: string): string => s.replace(/\r$/, '').normalize('NFC').trimEnd();
...
export const PatchParser = { parsePatch, deriveNewContentsFromChunks };
