...
## 2026-09-09 统一17家演示与首屏缩放修复

- 原因：旧版本把17家公开资料演示和8家虚拟供需演示做成了两套入口；首屏使用默认900×560尺寸、默认相机先显示，布局停止时再执行220ms的zoomToFit，造成可见缩放跳变。
- 现在仅“体验演示”一个入口，始终17家原企业；同行业11对、模拟供需25对。模拟供需是独立字段，不改变原始公开资料；切换关系不替换节点身份。
- 首屏测量和配置移至布局阶段；先预热，再进行0ms适配，下一动画帧显示画布。后续布局停止不重复适配，缓存相机在显示前恢复。手动适配保留。
- 92项回归测试通过，含初始化适配只执行一次、缓存相机恢复、同一17个身份、原资料不变、真实来源忽略模拟字段及现有园区服务61项。原有园区测试仍有React act提示，无失败。
- renderer类型检查、针对性eslint、CSV生成一致性、code-map与diff检查通过。最终生产构建结果见本轮交付回复。
- 本次没有进行真机验证；之前布局性能记录不作为此次初始化策略的性能验收结论。
