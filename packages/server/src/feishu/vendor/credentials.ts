 ...
 * https://github.com/NSIETeam/otto-new.git
 ...
 *     这个工程改动较大，目前未实现。
 *
 *   - 仅接受经过认证加密的 AES-256-GCM 凭证（带 `gcm:` 前缀）。历史的
 *     AES-256-CBC 格式无完整性校验，会让能写入凭证文件的攻击者篡改 /
 *     padding-oracle 后经 JSON.parse 注入，因此已彻底移除：任何非 GCM
