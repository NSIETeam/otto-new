...
export interface AgentContext {
  type: 'main' | 'sub';
  agentId?: string;  // subAgent的ID
  taskDescription?: string; // subAgent的任务描述
}
...
export class ToolCallEvent {
  ...
  error_type?: string;
  prompt_id: string;
  response_length: number; // 返回给大模型的内容长度
  ...
  constructor(call: CompletedToolCall) {
    ...
    // 使用公共函数计算返回内容长度
    this.response_length = calculateResponseLength(call.response.responseParts);
  }
}
...
export class ApiResponseEvent {
  ...
  cache_write_input_tokens: number; // 缓存写入的token数
  cache_read_input_tokens: number; // 缓存命中的token数
  thoughts_token_count: number;
  tool_token_count: number;
  total_token_count: number;
  credits_usage: number; // 添加credits消耗字段
