...
async function requestJson(
  ...
  timer.unref?.();
  try {
    const response = await fetchImpl(target, { ...init, signal: controller.signal, redirect: 'error' });
...
interface PendingWeCom { scode: string; requestedScopes: readonly string[]; expiresAtMs: number }

function isPending<T extends { expiresAtMs: number }>(
  entries: Map<string, T>, pairingId: string, pending: T, now: number,
): boolean {
  if (entries.get(pairingId) !== pending) return false;
  if (now < pending.expiresAtMs) return true;
  entries.delete(pairingId);
  return false;
}

const ENDED_PAIRING = { status: 'denied', reason: 'channel pairing expired or cancelled' } as const;
...
export class WeComOfficialQrPairingBrokerV1 implements ChannelPairingBrokerV1 {
  ...
  async poll(pairingId: string): Promise<ChannelPairingBrokerStatus> {
    ...
    if (!isPending(this.pending, pairingId, pending, this.now())) return ENDED_PAIRING;
    const response = await requestJson(
      this.fetchImpl,
    ...
    if (['failed', 'fail', 'expired', 'denied', 'cancelled'].includes(status)) {
      this.pending.delete(pairingId);
      return ENDED_PAIRING;
    }
    ...
    return {
      ...
      authorization: {
        ...
        botName: string(bot.botname) || 'Otto',
...
interface PendingDingTalk { deviceCode: string; requestedScopes: readonly string[]; pollAfterMs: number; expiresAtMs: number }
