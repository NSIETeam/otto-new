...
function dataStatisticsAssignmentView(row: ParkDataStatisticsAssignmentRow): ParkDataStatisticsAssignmentView {
  return {
    id: row.id,
    taskId: row.task_id,
    organizationId: row.organization_id,
...
function dataStatisticsTaskView(
  ...
  try {
    ...
    if (Array.isArray(parsed)) fields = parsed.filter((item): item is string => typeof item === 'string');
  } catch { /* old/invalid field metadata is treated as empty */ }
  return {
    id: row.id,
    parkId: row.park_id,
    title: row.title,
...
function statisticsTaskRow(taskId: string): ParkDataStatisticsTaskRow {
  const row = getDB().prepare('SELECT * FROM park_data_statistics_tasks WHERE id = ?').get(taskId) as
    | ParkDataStatisticsTaskRow | undefined;
  if (!row) throw new Error('数据统计任务不存在');
  return row;
}

function statisticsAssignmentRows(taskId: string): ParkDataStatisticsAssignmentView[] {
...
function assertParkAdmin(accountId: string): { account: AccountView; park: ParkView } {
  const account = getAccount(accountId);
  if (!account?.isAdmin || account.status !== 'active') throw new Error('只有园区管理员可以管理数据统计');
  const park = getParkForOrganization(account.organizationId);
  if (!park || park.adminOrganizationId !== account.organizationId) throw new Error('当前企业不是园区管理方');
  if (!getOrganizationFeatures(account.organizationId).park_service) throw new Error('园区服务功能已关闭');
  return { account, park };
}

function assertTaskMember(taskId: string, accountId: string): {
  ...
  const task = statisticsTaskRow(taskId);
...
export function createParkDataStatisticsTask(input: {
  ...
  const { account, park } = assertParkAdmin(input.createdByAccountId);
  ...
  const tenantOrganizations = listParkTenantOrganizations(park.id);
  ...
  const recipients = wanted.map((organization) => {
    const ceo = listAccounts(organization.id).find((item) => item.isAdmin && item.status === 'active');
  ...
  try {
    ...
    logAudit('park_statistics_create', account.employeeId, `${id} delivered to ${recipients.length} enterprise(s)`, account.organizationId);
    database.exec('COMMIT');
  } catch (error) {
    database.exec('ROLLBACK');
    throw error;
  }
  const row = statisticsTaskRow(id);
  return { task: dataStatisticsTaskView(row, statisticsAssignmentRows(id)), recipientCount: recipients.length };
}
...
export function listParkDataStatisticsTasks(accountId: string): ParkDataStatisticsTaskView[] {
  ...
  const rows = account.isAdmin && park?.adminOrganizationId === account.organizationId
    ? getDB().prepare('SELECT * FROM park_data_statistics_tasks WHERE admin_organization_id = ? ORDER BY created_at DESC').all(account.organizationId)
    : getDB().prepare(
  ...
  return (rows as ParkDataStatisticsTaskRow[]).map((row) => {
    const assignments = statisticsAssignmentRows(row.id);
...
export function markParkDataStatisticsRead(taskId: string, accountId: string): ParkDataStatisticsAssignmentView {
  const { assignment } = assertTaskMember(taskId, accountId);
  getDB().prepare(
    `UPDATE park_data_statistics_assignments SET read_at = COALESCE(read_at, datetime('now')), updated_at = datetime('now') WHERE id = ?`,
  ).run(assignment.id);
  return statisticsAssignmentRows(taskId).find((item) => item.id === assignment.id)!;
}
...
export function getParkDataStatisticsTemplate(taskId: string, accountId: string): {
  ...
  const { task } = assertTaskMember(taskId, accountId);
...
export function delegateParkDataStatistics(taskId: string, accountId: string, assigneeAccountId: string): ParkDataStatisticsAssignmentView {
  const { assignment, account } = assertTaskMember(taskId, accountId);
  if (!account.isAdmin || account.id !== assignment.ceoAccountId) throw new Error('只有企业负责人可以分派数据统计任务');
  const assignee = getAccount(assigneeAccountId, account.organizationId);
...
export function submitParkDataStatisticsDraft(taskId: string, accountId: string, responseData: Record<string, string>): ParkDataStatisticsAssignmentView {
  const { task, assignment, account } = assertTaskMember(taskId, accountId);
...
export function remindParkDataStatistics(taskId: string, adminAccountId: string): ParkDataStatisticsTaskView {
  const { account } = assertParkAdmin(adminAccountId);
  const task = statisticsTaskRow(taskId);
  const park = getPark(task.park_id);
  ...
  return dataStatisticsTaskView(task, statisticsAssignmentRows(taskId));
}
...
export function returnParkDataStatistics(taskId: string, adminAccountId: string, organizationId: string, reason: string): ParkDataStatisticsAssignmentView {
  ...
  const row = getDB().prepare('SELECT id FROM park_data_statistics_assignments WHERE task_id = ? AND organization_id = ?').get(taskId, organizationId) as { id: string } | undefined;
  ...
  return statisticsAssignmentRows(taskId).find((item) => item.id === row.id)!;
}
