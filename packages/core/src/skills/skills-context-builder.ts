...
export class SkillsContextBuilder {
  ...
  constructor(projectRoot?: string) {
    ...
    this.skillsDir = path.join(homeDir, '.otto-user', 'skills');
    this.marketplaceDir = path.join(homeDir, '.otto-user', 'marketplace');
    this.userGlobalSkillsDir = path.join(homeDir, '.otto-user', 'skills');
  ...
  private generateSummary(skills: SkillInfo[]): string {
    ...
    lines.push('1. AI reads the built-in "ppt-creator" skill');
    lines.push('2. AI defines the communication job, narrative arc, and a unique visual motif for this deck');
    lines.push('3. AI builds three benchmark slides with custom HTML/CSS/SVG and checks real screenshots');
    lines.push('4. After the benchmark passes, AI renders every 1920x1080 slide locally and packages the PNGs with Node/PptxGenJS');
    lines.push('5. generate_document is only a clearly disclosed speed fallback, never the default for a high-aesthetic deck');
    lines.push('6. No Python script, fixed template, cloud PPT service, login, or upload is used');
    lines.push('');
    lines.push('❌ WRONG approach:');
    lines.push('1. AI routes normal PPT generation to a marketplace Python script');
    lines.push('2. AI uploads content or requires an Otto web/server URL');
    lines.push('3. AI repeats one title-and-card template across every slide');
    lines.push('4. AI returns Markdown, HTML, or mock screenshots instead of a verified PPTX');
    lines.push('```');
    lines.push('');
  ...
  /**
   * Get user global skills (from ~/.otto/skills/)
   */
  ...
  /**
   * Get project skills (from {project}/.otto/skills/)
   */
