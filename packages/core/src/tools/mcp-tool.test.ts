...
describe('DiscoveredMCPTool', () => {
  ...
  describe('execute', () => {
    it('should call mcpTool.callTool with correct parameters and format display output', async () => {
      ...
      const toolResult: ToolResult = await tool.execute(params, new AbortController().signal);
    ...
    it('should propagate rejection if mcpTool.callTool rejects', async () => {
      ...
      await expect(tool.execute(params, new AbortController().signal)).rejects.toThrow(expectedError);
    });

    it('should throw error if AbortSignal is already aborted before execution', async () => {
      const tool = new DiscoveredMCPTool(
        mockCallableToolInstance,
        serverName,
        serverToolName,
      ...
      await expect(tool.execute(params, abortController.signal)).rejects.toThrow(
        'MCP tool execution was cancelled by user'
      );
    ...
    it('should cancel execution if AbortSignal is aborted during execution', async () => {
      const tool = new DiscoveredMCPTool(
        mockCallableToolInstance,
        serverName,
        serverToolName,
    ...
    it('should timeout if execution takes longer than configured timeout', async () => {
      const customTimeout = 100; // 100ms 超时
      const tool = new DiscoveredMCPTool(
        mockCallableToolInstance,
        serverName,
        serverToolName,
