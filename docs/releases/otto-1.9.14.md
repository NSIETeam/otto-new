...
1.9.14 基于 `internal` 产品基线，合并经过审计的企业服务、桌面端、新 UI、园区服务、模型调用、数据保护和打包修复。发布流程保持“先锁定双仓发布前状态、再创建草稿和验证、最后依次公开与切换镜像”的顺序。签名或服务器部署在公开前失败时不会向用户暴露新版本；公开后镜像失败则按锁定快照恢复此前 `latest`，已经公开的目标 Release 不会退回草稿，版本号也不会复用。
...
- Windows Authenticode、macOS Developer ID/公证、企业包签名和服务器部署通过后，先公开并复核 `NSIETeam/otto-new`，再把 `Felix201209/otto-releases` 作为最后一个 Release 端点公开，随后才切换更新镜像。
