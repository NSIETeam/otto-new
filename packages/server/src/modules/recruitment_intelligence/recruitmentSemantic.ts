...
/** Legacy wire values describe material support, NEVER human verification. */
...
export interface RecruitmentSemanticEvaluation {
  coordination?: { claimId: string; status: 'recorded' | 'unconfirmed'; message: string };
  summary: string;
  overallScore: number;
  ...
  outputTokens: number;
  createdAt: string;
  /** Captured by Otto at the model boundary, not accepted from model JSON. Optional for old archives. */
  assessmentContext?: RecruitmentAssessmentContext;
  /** Main-process metadata, never supplied by the model. Counts belong to runId, not each reuse. */
  execution?: {
    runId: string;
    requestId?: string;
...
export interface RecruitmentAssessmentContext {
  schemaVersion: 1;
  jobFingerprint: string;
  materialFingerprint: string;
  enterpriseFingerprint: string;
  materialScope: Array<'resume' | 'interview' | 'work_sample'>;
  modelId: string;
}

export interface RecruitmentSemanticAnalysisInput {
  /** Main-process coordination hint, not model content or an authority grant. */
  sharedIntake?: { scopeId: string; jobId: string; itemId: string; scopeToken: string; headerToken: string };
  candidateId: string;
  jobTitle: string;
  jobDescription: string;
  redactedResume: string;
  /** False when redactedResume is only an interview-only prompt placeholder. */
