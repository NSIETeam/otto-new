...
const PREVIEW_DIRECTORIES = ['shots', 'slides', 'previews', 'preview', 'thumbs'];
...
function unsupported(filePath: string, error: string): LocalArtifactPreviewResult {
  return {
    ok: false,
    kind: 'unsupported',
  ...
  for (const directoryName of PREVIEW_DIRECTORIES) {
    ...
    if (slides.length > 0) {
      return {
        ...
        mimeType:
          extension === '.pptx' ? PRESENTATION_MIME : 'application/vnd.ms-powerpoint',
        slides,
      };
    }
  }
