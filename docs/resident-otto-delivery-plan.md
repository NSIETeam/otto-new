...
| Durable workflow state changes produce restart-safe, idempotent chat milestones; unchanged states and internal timestamp churn do not notify, and the worker is a named zero-cost recurring task | `channelWorkflowMilestones.ts`, `managedChannelPlatform.ts`, `server.ts`, focused tests | Automated |
