...
const storage = {
  ...
  unprotect: (value: string) =>
    Buffer.from(value.slice(10), 'base64').toString(),
};
function endpoint() {
...
describe('encrypted identity continuity after endpoint migration', () => {
  it('registers the original device through the NEW endpoint after a real client login', async () => {
      ...
      async (input, init) => {
        ...
        if (url.endsWith('/health'))
          body = {
            status: 'ok',
            apiVersion: 4,
            capabilities: ['password_auth', 'e2ee_private_messages_v1'],
          };
        else if (url.endsWith('/auth/login'))
          body = {
            token: 'synthetic-token',
            account: { id: 'alice', organizationId: 'org' },
            expiresAt: '2099-01-01',
          };
        else if (url.endsWith('/e2ee/devices')) {
          expect(JSON.parse(String(init?.body)).deviceId).toBe(device.deviceId);
          body = {
            device: { ...device, approvalState: 'approved', revokedAt: null },
          };
        } else if (url.includes('/key-transparency?'))
          body = {
            transparency: {
              accountId: 'alice',
              headSequence: 0,
    ...
    expect((await client.ensureE2eeDeviceReady()).deviceId).toBe(
      device.deviceId,
    );
  ...
  it.each(legacyUrls)(
    'retains device identity and every original byte from %s after restart',
    (origin) => {
      const original = endpoint();
      ...
      const restarted = new EnterpriseE2eeCrypto(
        new EnterpriseE2eeKeyVault({ directory: original.root, ...storage }),
      );
      const scope = restarted.resolveServerScope(newUrl, 'alice');
      expect(scope).toBe(origin);
      expect(restarted.localDevice(scope, 'alice')).toEqual(
        original.crypto.localDevice(origin, 'alice'),
      );
      expect(
        original.vault.loadOrCreate(scope, 'alice').historical[0].deviceId,
  ...
  it('decrypts old signed messages with the retained key, without contacting the old endpoint', () => {
    ...
    const sent = alice.crypto.encryptMessage({
      serverScope: oldUrl,
      organizationId: 'org',
    ...
    ).toBe('synthetic historical message');
    expect(() =>
      bob.crypto.decryptMessage({
        serverScope: scope,
  ...
  it('opens the existing encrypted MLS history and pending outbox under the original identity', async () => {
    const original = endpoint();
    const device = original.crypto.localDevice(oldUrl, 'alice');
    const store = new FileEnterpriseMlsMessageHistory({
      directory: join(original.root, 'history'),
    ...
    const message = {
      ...
      e2ee: true as const,
      e2eeProtocol: 'mls10-openmls-0.8' as const,
    };
    await store.put(identity, 'bob', message);
    const reopened = new FileEnterpriseMlsMessageHistory({
      directory: join(original.root, 'history'),
      secureStorage: storage,
    });
    const migrated = {
      ...identity,
      serverUrl: original.crypto.resolveServerScope(newUrl, 'alice'),
    };
    expect(await reopened.list(migrated, 'bob')).toEqual(
      await store.list(identity, 'bob'),
    );
    expect((await reopened.list(migrated, 'bob'))[0].content).toBe(
      message.content,
    );
    expect(await reopened.pendingOutgoing(migrated, 'bob')).toHaveLength(1);
    expect(
      await reopened.list({ ...migrated, organizationId: 'other' }, 'bob'),
    ).toEqual([]);
  });
  ...
  it('keeps the old transparency pin and rejects a restored-server rollback', () => {
    ...
    const entryHash = createHash('sha256')
      ...
      .digest('hex');
    original.crypto.verifyAndPinKeyTransparency({
      serverScope: oldUrl,
      organizationId: 'org',
      view: {
        accountId: 'alice',
    ...
    expect(() =>
      original.crypto.verifyAndPinKeyTransparency({
        serverScope: original.crypto.resolveServerScope(newUrl, 'alice'),
        organizationId: 'org',
        view: {
          accountId: 'alice',
          headSequence: 0,
  ...
  it('rejects a keyring copied from a different account', () => {
    ...
    const digest = (account: string) =>
      `${createHash('sha256').update(`${oldUrl}\0${account}`).digest('hex')}.keyring`;
    writeFileSync(
      join(original.root, digest('alice')),
      readFileSync(join(original.root, digest('bob'))),
    );
    expect(() => original.crypto.resolveServerScope(newUrl, 'alice')).toThrow(
      /keyring/,
    );
  ...
  it('does not silently create a replacement when the legacy keyring is corrupt', () => {
    ...
    const file = join(
      original.root,
      `${createHash('sha256').update(`${oldUrl}\0alice`).digest('hex')}.keyring`,
    );
  ...
  it.each([
    ...
    `${newUrl}/tenant`,
    'http://101.200.190.204:7777',
    'https://101.200.190.204:8443',
    `${newUrl}?tenant=x`,
    'https://101.200.190.204.attacker.test:7777',
  ])('does not alias private or unapproved endpoint %s', (url) => {
    const original = endpoint();
    original.crypto.localDevice(oldUrl, 'alice');
