...
在整合快照 `469ec498`，独立诊断执行了 333 次 loopback HTTP 请求，三用户
邀请、授权入组、加入前历史隔离、退组拒绝和客户端/native 进程重建恢复共 8 阶段通过；
6 个原生进程正常退出、监听器关闭。采用真实 HTTP、设备签名和 MLS，身份、地图和
本地状态密钥为隔离夹具；不代表正式账号审批、OS 密钥库重登、SQLCipher 或三人 GUI。
另一个真实 Electron 43.2 / OS safeStorage 双人 UI 验证已完成发布、请求、接受、
加密发送与接收方解密断言；其最终截图调用超时，原脚本整体退出 1，不能登记为整套通过。
后续独立截图成功保留。真实高德公共地标接口通过的证据见下文，与 UI 的地图夹具区分。

本轮完整 CI `34315295265` 中 server 为 3,055 通过、2 失败、1 既有跳过，
失败是 CI 未编译当前源码的 Rust 测试二进制。随后在 CI 和 release 同时增加固定工具链、
`--locked` 的 native 构建前置步骤，不跳过 SQLite/PostgreSQL 的真实加密通信测试；
3 项前置合同先失败后通过，相关 38 项测试通过。最终仍须新源码完整 CI 通过才可发布。
