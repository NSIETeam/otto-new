...
export type PrivateDeploymentBootstrapPhase =
  'idle' | 'not_configured' | 'claiming' | 'activated' | 'failed';

export type PrivateDeploymentReadinessState =
  'ready' | 'ready_for_identity' | 'configuring' | 'degraded' | 'blocked';

export type PrivateDeploymentStepState =
  'ready' | 'configuring' | 'waiting_for_user' | 'action_required' | 'disabled';
...
function usableLicense(license: DeploymentLicenseView): boolean {
  return (
    USABLE_LICENSE_STATES.has(license.status) &&
...
export function buildPrivateDeploymentReadiness(
  ...
  const registered =
    license.id !== 'unlicensed' && license.status !== 'invalid';
  ...
  const modelGatewayEnabled = capability(
    source,
    'modelGateway',
    billingRequired,
  );
  ...
  const steps: PrivateDeploymentReadinessStep[] = [
    ...
    step(
      ...
      !updateEnabled
        ? 'disabled'
        : licenseUsable &&
            Boolean(source.runtimeConfiguration?.updateDistributionId)
          ? 'ready'
          : bootstrapBusy
            ? 'configuring'
            : 'action_required',
  ...
  const waitingForIdentity = steps.some(
    (item) =>
      item.id === 'account_identity' && item.state === 'waiting_for_user',
  );
...
export function normalizeControlBootstrapResponse(
  ...
  if (
    body.provisioningCommand === undefined ||
    body.provisioningCommand === null
  ) {
  ...
  const configuration: PrivateDeploymentRuntimeConfiguration = {
    ...
    updateDistributionId:
      typeof body.updateDistributionId === 'string' &&
      body.updateDistributionId.trim()
...
export function createPrivateDeploymentBootstrapCoordinator(
  ...
  const readiness = () =>
    buildPrivateDeploymentReadiness(services.getReadinessSource(current));
...
export function startPrivateDeploymentBootstrapRuntime(
  ...
  const registry = options.taskRegistry ?? new RecurringTaskRegistry();
  const stop = registry.register({
    name: 'enterprise.private-deployment-bootstrap',
    source: 'packages/server/src/modules/deployment_lifecycle/privateDeploymentBootstrap.ts',
    intervalMs,
