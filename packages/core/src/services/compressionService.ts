...
import {
  ...
  type CompressionInvariantValidation,
...
export interface CompressionResult {
  ...
  newHistory?: Content[];
  skipReason?: string;
  invariantValidation?: CompressionInvariantValidation;
}
...
export class CompressionService {
  ...
  /** L2 摘要缓存 */
  private summaryCache: Map<
    string,
    { summary: string; history: Content[]; timestamp: number }
  > = new Map();
  ...
  constructor(config: CompressionServiceConfig = {}) {
    ...
    this.compressionPreserveThreshold =
      config.compressionPreserveThreshold ?? 0.3;
   ...
   */
  private wouldOrphanFunctionResponse(
    history: Content[],
    cutIndex: number,
  ): boolean {
    ...
    if (
      left.role === MESSAGE_ROLES.MODEL &&
      left.parts?.some((p: CompressionPart) => p.functionCall)
    ) {
      if (
        right.role === MESSAGE_ROLES.USER &&
        right.parts?.some((p: CompressionPart) => p.functionResponse)
      ) {
        return true;
      }
    }
  ...
  private findToolCallBoundary(history: Content[], startIndex: number): number {
    ...
    if (startIndex >= history.length) {
      console.warn(
        `[findToolCallBoundary] startIndex (${startIndex}) >= history.length (${history.length}), no suitable boundary found`,
      );
      return -1; // 没有合适的压缩区间
    }

    console.log(
      `[findToolCallBoundary] Searching from index ${startIndex} to ${history.length - 1}, total history length: ${history.length}`,
    );
    ...
    const isProtectedTool = (part: CompressionPart): boolean => {
      ...
      return Boolean(
        toolName && CompressionService.PROTECTED_TOOLS.includes(toolName),
      );
    };
    ...
    for (let i = startIndex; i < history.length; i++) {
      ...
      if (msg.parts) {
        ...
        if (hasProtectedTool) {
          const toolName =
            protectedToolPart.functionResponse?.name ||
            protectedToolPart.functionCall?.name;
          console.log(
            `${msgInfo} - PROTECTED TOOL FOUND: ${toolName}, skipping this boundary`,
          );
        }
      ...
      if (msg.role === 'user') {
        ...
        if (i > 0) {
          ...
          if (prevMsg.role === MESSAGE_ROLES.MODEL && prevMsg.parts) {
            const hasFunctionCall = prevMsg.parts.some(
              (p: CompressionPart) => p.functionCall,
            );
            const hasToolResult = prevMsg.parts.some(
              (p: CompressionPart) => p.toolResult,
            );

            // 如果有functionCall但没有toolResult，这表示tool还未响应，继续寻找
            if (hasFunctionCall && !hasToolResult) {
              console.log(
                `${msgInfo} (user) - SKIP: previous model message has pending tool call`,
              );
        ...
        console.log(
          `${msgInfo} (user) - FOUND SUITABLE BOUNDARY at index ${i + 1}`,
        );
        return i + 1; // 压缩到这个user消息（包含），保留后面的内容
      } else {
        const partTypes =
          msg.parts
            ?.map((p: CompressionPart) => {
    ...
    console.warn(
      `[findToolCallBoundary] No suitable user message boundary found. Trying fallback strategy with model messages...`,
    );
    for (let i = history.length - 1; i >= startIndex; i--) {
      const msg = history[i];
      if (
        msg.role === MESSAGE_ROLES.MODEL &&
        msg.parts?.some((p: CompressionPart) => p.text)
      ) {
        ...
        if (this.wouldOrphanFunctionResponse(history, candidateBoundary)) {
          console.log(
            `[findToolCallBoundary] SKIP fallback at index ${candidateBoundary}: would orphan functionResponse, continuing search`,
          );
          continue;
        }
        // 找到一个包含text的model消息，这也是一个合理的切割点
        console.log(
          `[findToolCallBoundary] Found fallback boundary at index ${candidateBoundary} (model message with text)`,
        );
    ...
    if (startIndex > 0) {
      ...
      if (this.wouldOrphanFunctionResponse(history, startIndex)) {
        ...
        if (adjustedIndex < history.length) {
          console.warn(
            `[findToolCallBoundary] Adjusting startIndex from ${startIndex} to ${adjustedIndex} to avoid orphaning functionResponse`,
          );
          return adjustedIndex;
        }
      }
      console.warn(
        `[findToolCallBoundary] Using fallback boundary at startIndex: ${startIndex}`,
      );
      return startIndex;
    }
  ...
  private validateAndCleanHistory(history: Content[]): Content[] {
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      if (!msg.parts || msg.parts.length === 0) {
        console.warn(
          `[CompressionService] Found message with empty parts array at index ${i}. Removing this message.`,
        );
        hasEmptyParts = true;
      }
      ...
      if (msg.parts && msg.parts.length > 0) {
        for (const part of msg.parts) {
          ...
          if (partAny.toolResult) {
            const toolResultId = (
              partAny.toolResult as { toolUseId?: string } | undefined
            )?.toolUseId;
            if (!toolResultId || !toolCallStack[toolResultId]) {
              // 这是一个孤立的tool_result，需要移除整个消息
              console.warn(
                `[CompressionService] Found orphaned tool_result with ID: ${toolResultId}. Removing this message.`,
              );
          ...
          if (partAny.functionResponse) {
            ...
            if (!isValid) {
              console.warn(
                `[CompressionService] Found orphaned functionResponse (name=${responseName}, id=${responseId}). Removing this message.`,
              );
              hasInvalidToolResult = true;
              break;
            }
    ...
    if (cleanedHistory.length < history.length) {
      console.log(
        `[CompressionService] Cleaned history: removed ${history.length - cleanedHistory.length} invalid messages`,
      );
    }
  ...
  private ensureAlternatingRoles(history: Content[]): Content[] {
    ...
    if (history[0].role !== MESSAGE_ROLES.USER) {
      ...
      console.warn(
        `[ensureAlternatingRoles] Inserted placeholder user message at start (first message was ${history[0].role})`,
      );
    }
      ...
      if (prev && prev.role === msg.role) {
        ...
        const fillerRole =
          msg.role === MESSAGE_ROLES.USER
            ...
            : MESSAGE_ROLES.USER;
        const fillerText =
          fillerRole === MESSAGE_ROLES.MODEL
            ? 'Understood.'
        ...
        console.warn(
          `[ensureAlternatingRoles] Inserted placeholder ${fillerRole} message before index ${i} to fix consecutive ${msg.role} messages`,
        );
      }
    ...
    if (result.length !== history.length) {
      console.log(
        `[ensureAlternatingRoles] Fixed role alternation: ${history.length} -> ${result.length} messages`,
      );
    }
  ...
  private trimTrailingToolCalls(history: Content[]): Content[] {
    ...
    const hasToolParts = (msg: Content): boolean => {
      ...
      return msg.parts.some(
        (p: CompressionPart) =>
          p.functionCall || p.functionResponse || p.toolResult,
      );
    };
        ...
        if (p.functionCall) {
          ...
          if (args) {
            if (args.file_path || args.absolute_path)
              argSummary = ` on "${args.file_path || args.absolute_path}"`;
              ...
              argSummary = `: \`${args.command.substring(0, 80)}\``;
          }
          toolSummaryParts.push(
            `- Called \`${p.functionCall.name}\`${argSummary}`,
          );
        }
    ...
    console.log(
      `[trimTrailingToolCalls] Trimmed ${trimmedMessages.length} trailing tool messages (${toolSummaryParts.length} tool operations) and replaced with text summary`,
    );
  ...
  async shouldCompress(
    ...
    try {
      ...
      console.warn(
        `Could not determine token count for model ${model}. Error: ${getErrorMessage(error)}`,
      );
      return { shouldCompress: false };
    }
    ...
    // 检查是否超过压缩阈值
    const threshold =
      this.compressionTokenThreshold * tokenLimit(model, config);
  ...
  async compressHistory(
    ...
    try {
      const invariantSnapshot = captureCompressionInvariants(
        history,
        this.skipEnvironmentMessages,
      );
      // 获取或计算原始token数量
      ...
      if (finalOriginalTokenCount === undefined) {
        const originalTokenResult = await this.shouldCompress(
          history,
          model,
          geminiClient.getContentGenerator(),
          false,
          config,
        );
      ...
      // 分离环境信息和实际对话历史
      const environmentMessages = history.slice(
        0,
        Math.min(this.skipEnvironmentMessages, history.length),
      );
      ...
      const preserveRatio =
        overridePreserveRatio ?? this.compressionPreserveThreshold;
      ...
      console.log(
        `[compressHistory] Compression plan: conversationHistory.length=${conversationHistory.length}, preserveRatio=${preserveRatio}, initialCompressBeforeIndex=${compressBeforeIndex}`,
      );

      // 寻找最近的完整工具调用对边界，统一处理主agent和subAgent场景
      compressBeforeIndex = this.findToolCallBoundary(
        conversationHistory,
        compressBeforeIndex,
      );
      ...
      if (compressBeforeIndex === -1) {
        console.warn(
          `[compressHistory] Could not find suitable compression boundary. Conversation history structure may prevent compression.`,
        );
        console.log(
          `[compressHistory] Last 5 messages in conversationHistory:`,
        );
        for (
          let i = Math.max(0, conversationHistory.length - 5);
        ...
        if (force) {
          ...
          console.warn(
            `[compressHistory] Force compression: no clean boundary found, compressing entire conversation history.`,
          );
      ...
      const historyToCompress = conversationHistory.slice(
        0,
        compressBeforeIndex,
      );
      ...
      while (historyToKeep.length > 0) {
        ...
        if (firstMessage.role === MESSAGE_ROLES.MODEL) {
          const hasOnlyToolResult = firstMessage.parts?.every(
            (part: CompressionPart) =>
              'toolResult' in part && !('text' in part),
          );
          if (
            hasOnlyToolResult &&
            firstMessage.parts &&
        ...
        if (firstMessage.role === MESSAGE_ROLES.USER) {
          const hasFunctionResponse = firstMessage.parts?.some(
            (part: CompressionPart) => part.functionResponse,
          );
        ...
        if (shouldRemove) {
          console.warn(
            `[CompressionService] Removing orphaned tool result/response at start of historyToKeep (role=${firstMessage.role})`,
          );
      ...
      const compressionPrompt =
        'First, reason in your <analysis> scratchpad. Then, generate the <summary> containing the <state_snapshot>.';
      ...
      if (isModelSwitchCompression) {
        ...
        if (historyForCompression.length > maxHistoryLength) {
          ...
          console.log(
            `[CompressionService] Model switch compression: limiting history from ${historyForCompression.length} to ${historyForCompressionRequest.length} messages`,
          );
        }
      }
      ...
      for (const msg of historyForCompressionRequest) {
        if (
          ...
          const textParts =
            msg.parts?.filter((part: CompressionPart) => 'text' in part) || [];
      ...
      while (ptlRetryCount <= MAX_PTL_RETRIES) {
        ...
        try {
          ...
          const isPTL =
            (errorMsg.includes('prompt') && errorMsg.includes('too long')) ||
          ...
          const envCount = Math.min(
            this.skipEnvironmentMessages,
            currentPurifiedHistory.length,
          );
          const nonEnvMessages = currentPurifiedHistory.slice(envCount);
          const truncateCount = Math.max(
            1,
            Math.ceil(nonEnvMessages.length * PTL_TRUNCATE_RATIO),
          );
          ...
          if (remainingMessages.length === 0) {
            throw new Error(
              'PTL: No messages left after truncation, cannot compress',
            );
          }

          // 确保第一条消息是 user（API 要求）
          let newHistory = [
            ...currentPurifiedHistory.slice(0, envCount),
            ...remainingMessages,
          ];
          if (
            newHistory.length > 0 &&
            newHistory[0].role !== MESSAGE_ROLES.USER
          ) {
            newHistory = [
              {
                role: MESSAGE_ROLES.USER,
                parts: [
                  {
                    text: '[Earlier conversation context was truncated due to length]',
                  },
                ],
              },
              ...newHistory,
          ...
          console.warn(
            `[CompressionService] PTL retry ${ptlRetryCount}/${MAX_PTL_RETRIES}: truncated ${truncateCount} oldest messages, ${currentPurifiedHistory.length} remaining`,
          );
        }
      }
      ...
      if (!compressionResponse) {
        throw new Error(
          'Compression failed: no response received after PTL retries',
        );
      }

      console.log(
        `[CompressionService] Compression response received${ptlRetryCount > 0 ? ` (after ${ptlRetryCount} PTL retries)` : ''}:`,
        {
          hasCandidates: !!compressionResponse.candidates,
          candidatesLength: compressionResponse.candidates?.length,
          firstCandidateFinishReason:
            compressionResponse.candidates?.[0]?.finishReason,
          hasContent: !!compressionResponse.candidates?.[0]?.content,
          partsCount:
            compressionResponse.candidates?.[0]?.content?.parts?.length || 0,
          partTypes: compressionResponse.candidates?.[0]?.content?.parts?.map(
            (p: CompressionPart) => {
      ...
      if (summary.length < rawSummaryLength) {
        console.log(
          `[CompressionService] Stripped analysis scratchpad: ${rawSummaryLength} -> ${summary.length} chars`,
        );
      }
...
`;
      ...
      if (!invariantValidation.valid) {
        return {
          success: false,
          error: `Compression invariant validation failed: ${invariantValidation.missing.join(', ')}`,
          invariantValidation,
        };
      }
        ...
        const partTypes =
          msg.parts
            ?.map((p: CompressionPart) => {
              ...
              if (p.toolResult)
                return `toolResult(${(p.toolResult as { toolUseId?: string }).toolUseId})`;
              return 'unknown';
            })
            .join(',') || 'no-parts';
        console.log(`  [${i}] role=${msg.role}, parts=[${partTypes}]`);
      ...
      try {
        ...
        console.warn(
          `Could not determine compressed history token count. Error: ${getErrorMessage(error)}`,
        );
        return {
          success: false,
      ...
      console.log(
        `[CompressionService] Compression completed: ${finalOriginalTokenCount} -> ${newTokenCount} tokens`,
      );
  ...
  async tryCompress(
    ...
    if (!force && this.isCircuitBreakerTripped()) {
      console.warn(
        `[CompressionService] Circuit breaker tripped: ${this.consecutiveAutoCompressFailures} consecutive failures. Skipping auto-compress. Use /compress to force.`,
      );
      return null;
    }

    // 检查是否需要压缩
    const shouldCompressResult = await this.shouldCompress(
      history,
      model,
      geminiClient.getContentGenerator(),
      force,
    ...
    try {
      ...
      const result = await retryWithBackoff(
        async () => {
          ...
          if (!innerResult.success && !innerResult.skipReason) {
            throw new Error(
              innerResult.error || 'Compression failed without specific error',
            );
          }
        ...
        },
        {
          ...
          shouldRetry: (error) => {
            console.warn(
              `[CompressionService] Compression attempt failed: ${error.message}. Retrying...`,
            );
            return true;
          },
        },
      ...
      if (result && result.success) {
        this.consecutiveAutoCompressFailures = 0;
        // Record the actual hierarchy level selected by the client.
        if (result.compressionInfo) {
          this.recordCompressionStat(
            level,
            result.compressionInfo.originalTokenCount,
            result.compressionInfo.newTokenCount,
          );
        }
      }
      ...
      if (!force) {
        ...
        console.warn(
          `[CompressionService] Auto-compress failed. Consecutive failures: ${this.consecutiveAutoCompressFailures}/${this.maxConsecutiveFailures}`,
        );
      }
      throw error;
    }
  ...
  async compressToFit(
    ...
    return await retryWithBackoff(
      async () => {
        console.log(
          `[CompressionService] compressToFit called: ${currentModel} → ${targetModel}${knownTokenCount ? ` (knownTokenCount: ${knownTokenCount})` : ''}`,
        );
        ...
        if (currentTokenCount === undefined) {
          try {
            ...
            console.warn(
              `[CompressionService] Could not count tokens for model switch check: ${getErrorMessage(error)}`,
            );
        ...
        if (currentTokenCount === undefined) {
          return {
            success: true,
            skipReason:
              'Unable to determine token count for model switch. Proceeding without compression.',
          };
        }

        console.log(
          `[CompressionService] Model Switch Check: Current Tokens=${currentTokenCount}, Target Limit=${targetLimit} (Safe=${safeLimit})`,
        );
        ...
        console.log(
          `[CompressionService] History too large for target model ${targetModel}. Triggering aggressive compression.`,
        );
        ...
        // 但我们还是尽力而为
        requiredRatio = Math.max(
          0.05,
          Math.min(requiredRatio, this.compressionPreserveThreshold),
        );

        console.log(
          `[CompressionService] Dynamic compression ratio calculated: ${requiredRatio.toFixed(4)} (Available: ${availableForHistory}, Current: ${currentTokenCount})`,
        );
        ...
        if (!result.success && !result.skipReason) {
          throw new Error(
            result.error || 'Compression failed without specific error',
          );
        }
      ...
      },
      {
        ...
        shouldRetry: (error: unknown) => {
          console.warn(
            `[CompressionService] compressToFit attempt failed: ${getErrorMessage(error)}. Retrying...`,
          );
          return true;
        },
      },
  ...
  lightweightCleanup(history: Content[]): Content[] {
    ...
    // The active objective and the newest exchange are never rewritten by L1.
    // This protects a long current prompt from being truncated behind the user's
    // back and avoids damaging the latest function-call/result boundary.
    const protectedStart = Math.max(0, history.length - 4);
    let changed = false;
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      for (const part of msg.parts) {
        ...
        if (p.text && msg.role === MESSAGE_ROLES.USER) {
          ...
          const isEnv =
            text.includes('CRITICAL SYSTEM CONTEXT') ||
          ...
          if (!recentToolIndices.has(i)) {
            const toolName =
              ...
              (p.toolResult as { toolName?: string } | undefined)?.toolName ||
              'unknown';
            const compacted: CompressionPart = {
              functionResponse: {
                name: toolName,
                response: { output: '[L1 cleaned] ' + toolName },
              },
            };
            if (JSON.stringify(compacted).length < JSON.stringify(p).length) {
              changed = true;
              cleanParts.push(compacted);
            } else {
              cleanParts.push(p);
            }
          } else {
  ...
  shouldPreventiveCompress(
    ...
    // Find the newest function call. Compression is safe only if a later user
    // message contains its response. A final model explanation may follow that
    // response, so requiring the response to be the last history item would
    // leave preventive compaction effectively unreachable between user turns.
    let lastFunctionCallIndex = -1;
    for (let i = history.length - 1; i >= 0; i--) {
      const msg = history[i];
      if (
        msg.role === MESSAGE_ROLES.MODEL &&
        msg.parts?.some((p: CompressionPart) => p.functionCall)
      ) {
        lastFunctionCallIndex = i;
        break;
      }
    }
    if (lastFunctionCallIndex < 0) return false;

    const hasCompletedBoundary = history
      .slice(lastFunctionCallIndex + 1)
      .some(
        (msg) =>
          msg.role === MESSAGE_ROLES.USER &&
          msg.parts?.some(
            (p: CompressionPart) => p.functionResponse || p.toolResult,
          ),
      );
    if (!hasCompletedBoundary) return false;

    const estCost = 500;
    const estSave = tokenCount * 0.3;
    if (estSave <= estCost) {
      console.log(
        '[L2] Skip: expected saving ' + estSave + ' <= cost ' + estCost,
      );
      return false;
    }
  ...
  getCachedSummary(
    history: Content[],
  ): { summary: string; history: Content[] } | undefined {
