...
6. 生成全部逐页 PNG 和缩略图总览；
7. 逐页 100% 检查并返工；
8. 最后才用 PptxGenJS 组装 `.pptx`，执行结构检查，并把逐页 PNG 留给 Otto 内置预览。不要自行启动 WPS、PowerPoint 或 Keynote。
...
6. 写出后检查文件存在、大小合理、页数正确，逐页 PNG 与 PPT 页数一致；由 Otto 在 App 内预览。只有用户主动点击“用其他应用打开”时，才能调用系统演示软件。
...
- 最终回复把 `.pptx` 登记为可点击交付物，只显示文件名，不用行内代码暴露绝对路径；点击后默认进入 Otto 内置逐页预览；
- 禁止生成完成后自行启动 WPS、PowerPoint 或 Keynote；“用其他应用打开”只能是用户主动触发的次级操作；
- 数字、引用和图片来源必须可追溯；
