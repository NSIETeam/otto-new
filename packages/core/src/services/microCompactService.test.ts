...
type TestFunctionPart = { functionCall?: Record<string, unknown>; functionResponse?: { response?: { output?: unknown } } };
...
describe('MicroCompactService', () => {
  ...
  describe('shouldMicroCompact', () => {
    ...
    it('should trigger when idle exceeds idleThresholdMinutes', () => {
      ...
      (service as unknown as { lastAssistantMessageTime: number }).lastAssistantMessageTime = Date.now() - (61 * 60 * 1000);
    ...
    it('should trigger when token usage is high AND idle > 6 minutes', () => {
      ...
      (service as unknown as { lastAssistantMessageTime: number }).lastAssistantMessageTime = Date.now() - (7 * 60 * 1000); // 7 分钟前
    ...
    it('should NOT trigger token-based path when idle <= 6 minutes', () => {
      ...
      (service as unknown as { lastAssistantMessageTime: number }).lastAssistantMessageTime = Date.now() - (5 * 60 * 1000);
      expect(service.shouldMicroCompact(0.95)).toBe(false);
    });

    it('should NOT trigger token-based path when ratio below threshold', () => {
      (service as unknown as { lastAssistantMessageTime: number }).lastAssistantMessageTime = Date.now() - (10 * 60 * 1000);
  ...
  describe('microCompactMessages', () => {
    const createHistoryWithToolResults = (): Content[] => [
      ...
      // tool call 1
      { role: 'model', parts: [{ functionCall: { name: 'read_file', args: { path: '/a.txt' }, id: 'call_1' } } as unknown as TestFunctionPart] },
      { role: 'user', parts: [{ functionResponse: { name: 'read_file', response: { output: 'File content A (very long)' }, id: 'call_1' } } as unknown as TestFunctionPart] },
      // tool call 2
      { role: 'model', parts: [{ functionCall: { name: 'search_file_content', args: { pattern: 'foo' }, id: 'call_2' } } as unknown as TestFunctionPart] },
      { role: 'user', parts: [{ functionResponse: { name: 'search_file_content', response: { output: 'Search results...' }, id: 'call_2' } } as unknown as TestFunctionPart] },
      // tool call 3 (non-compactable)
      { role: 'model', parts: [{ functionCall: { name: 'save_memory', args: { fact: 'test' }, id: 'call_3' } } as unknown as TestFunctionPart] },
      { role: 'user', parts: [{ functionResponse: { name: 'save_memory', response: { output: 'Saved' }, id: 'call_3' } } as unknown as TestFunctionPart] },
      // tool call 4
      { role: 'model', parts: [{ functionCall: { name: 'glob', args: { pattern: '*.ts' }, id: 'call_4' } } as unknown as TestFunctionPart] },
      { role: 'user', parts: [{ functionResponse: { name: 'glob', response: { output: 'file1.ts\nfile2.ts' }, id: 'call_4' } } as unknown as TestFunctionPart] },
      // tool call 5
      { role: 'model', parts: [{ functionCall: { name: 'read_file', args: { path: '/b.txt' }, id: 'call_5' } } as unknown as TestFunctionPart] },
      { role: 'user', parts: [{ functionResponse: { name: 'read_file', response: { output: 'File content B' }, id: 'call_5' } } as unknown as TestFunctionPart] },
    ];
    ...
    it('should clear old compactable tool results when idle', () => {
      ...
      // 模拟超过空闲阈值：直接修改内部状态
      (service as unknown as { lastAssistantMessageTime: number }).lastAssistantMessageTime = Date.now() - (61 * 60 * 1000); // 61 minutes ago
      ...
      const firstToolResponse = (history[3].parts![0] as TestFunctionPart).functionResponse;
      ...
      const nonCompactableResponse = (history[7].parts![0] as TestFunctionPart).functionResponse;
      expect(nonCompactableResponse.response.output).toBe('Saved');

      // Last compactable result should be kept
      const lastToolResponse = (history[11].parts![0] as TestFunctionPart).functionResponse;
    ...
    it('should not clear already-cleared results', () => {
      ...
      (history[3].parts![0] as TestFunctionPart).functionResponse.response = { output: CLEARED_TOOL_RESULT_MARKER };
    ...
    it('should skip environment messages', () => {
      ...
      const history: Content[] = [
        { role: 'user', parts: [{ functionResponse: { name: 'read_file', response: { output: 'env file' }, id: 'env_1' } } as unknown as TestFunctionPart] },
        { role: 'model', parts: [{ text: 'Got it!' }] },
        { role: 'model', parts: [{ functionCall: { name: 'read_file', args: { path: '/x.txt' }, id: 'call_x' } } as unknown as TestFunctionPart] },
        { role: 'user', parts: [{ functionResponse: { name: 'read_file', response: { output: 'X content' }, id: 'call_x' } } as unknown as TestFunctionPart] },
      ];
      ...
      const envResponse = (history[0].parts![0] as TestFunctionPart).functionResponse;
  ...
  describe('reset', () => {
    it('should reset lastAssistantMessageTime', () => {
      (service as unknown as { lastAssistantMessageTime: number }).lastAssistantMessageTime = 0;
