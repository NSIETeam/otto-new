...
## 2026-09-08：桌面及容量证据分层

两个 Electron 窗口用真实组件配合测试桥接访问真实 HTTP/认证/加密服务，可以证明界面字段和业务链路联通，但不能替代完整安装包 IPC 安全验收。独立布局脚本使用真实 OS safeStorage，认证窗口脚本的临时草稿适配器不充当加密存储证明。压力测试保持生产每账号每分钟 60 条限制，采用双方各 50 条；原生 MLS 客户端的队列使 100 个调用不等于 100 个服务端同时写入，结果中明确区分此范围。
