...
describe('fileUtils', () => {
  ...
  describe('detectFileType', () => {
    ...
    describe('DTS file intelligent detection', () => {
      it('should detect Linux device tree .dts file as text', async () => {
        const dtsContent = `/dts-v1/;

/ {
    model = "Test Board";
    compatible = "test,board", "generic-board";
    ...
    cpu0: cpu@0 {
        ...
        const testFile = path.join(tempRootDir, 'test.dts');
        actualNodeFs.writeFileSync(testFile, dtsContent);

        expect(await detectFileType(testFile)).toBe('text');
      });

      it('should detect Linux device tree .dtsi file as text', async () => {
...
&cpu0 {
    ...
    clock-frequency = <800000000>;
        ...
        const testFile = path.join(tempRootDir, 'test.dtsi');
        actualNodeFs.writeFileSync(testFile, dtsiContent);

        expect(await detectFileType(testFile)).toBe('text');
      });

      it('should detect device tree file with large copyright header as text', async () => {
 ...
 *
 * Very long copyright header that might exceed 512 bytes...
...
/ {
    compatible = "vendor,board";
        ...
        const testFile = path.join(tempRootDir, 'large-header.dts');
        actualNodeFs.writeFileSync(testFile, dtsContent);

        expect(await detectFileType(testFile)).toBe('text');
      });

      it('should detect binary audio DTS file as audio', async () => {
        // Mock binary DTS audio file content
        const binaryContent = Buffer.from([
          0x7F, 0xFE, 0x80, 0x01, // DTS sync word
          0x00, 0x00, 0x00, 0x00,
          0xFF, 0xEE, 0xDD, 0xCC,
          // More binary audio data...
        ]);
        const testFile = path.join(tempRootDir, 'audio.dts');
        actualNodeFs.writeFileSync(testFile, binaryContent);

        // Mock MIME type to return audio for this file
        mockMimeLookup.mockReturnValueOnce('audio/vnd.dts');

        expect(await detectFileType(testFile)).toBe('audio');
      });

      it('should handle empty DTS file gracefully', async () => {
        const testFile = path.join(tempRootDir, 'empty.dts');
      ...
      it('should handle DTS file read errors gracefully', async () => {
        const testFile = path.join(tempRootDir, 'error.dts');
        // File doesn't exist, should trigger read error
